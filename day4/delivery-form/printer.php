...
<head>
    ...
    <title>Printer Delivery Form</title>
...
<body>
    ...
    <form action="" method="post">
        ...
        <input type="text" name="fullname" placeholder="Vũ Minh Đức" class="<?= $fullnameErr ? 'input-error' : '' ?>" value="<?= $fullname ?>" />
        <?= $fullnameErr ? "<p class='smg-error'>{$fullnameErr}</p>" : '' ?>
        </div>

        <div class="product-item">
        <label>Số điện thoại: </label>
        <input type="text" name="phone" placeholder="0000-000-000" class="<?= $phoneErr ? 'input-error' : '' ?>" value="<?= $phone ?>" />
        <?= $phoneErr ? "<p class='smg-error'>{$phoneErr}</p>" : '' ?>
        </div>

        <div class="product-item">
        <label>Địa chỉ nhận hàng: </label>
        <input type="text" name="address" placeholder="Số nhà 00, Đường XX, Huyện YY, Tỉnh ZZ" class="<?= $addressErr ? 'input-error' : '' ?>" value="<?= $address ?>" />
